    ...
    try {
      ...
      if (userData.roleId) {
        try {
          ...
          this.logger.warn(`Role with ID ${userData.roleId} not found: ${error.message}`);
          // If specified role doesn't exist, fallback to finding a default role
          const roles = await this.roleService.findAll();
          this.logger.log(`Available roles: ${roles.map(r => `${r.id}:${r.name}`).join(', ')}`);
          
          const userRole = roles.find((item) => item.name === 'Người dùng' || item.name === 'user');
          if (!userRole) {
            throw new Error('Neither specified role nor default user role found');
          }
        ...
        const roles = await this.roleService.findAll();
        this.logger.log(`Looking for default role among: ${roles.map(r => `${r.id}:${r.name}`).join(', ')}`);
